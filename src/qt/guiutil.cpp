...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("koreatruemoney"))
        return false;
...
{
    // Convert koreatruemoney:// to koreatruemoney:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("koreatruemoney://"))
    {
        uri.replace(0, 12, "koreatruemoney:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KoreaTrueMoney.lnk";
}
...
{
    return GetAutostartDir() / "koreatruemoney.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        optionFile << "Name=KoreaTrueMoney\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KoreaTrueMoney-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  koreatruemoney-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("KoreaTrueMoney-Qt"));
